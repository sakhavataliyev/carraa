          ...
          <div class="card border shadow-xs mb-4">
            <div class="card-header border-bottom pb-0">
              <div class="d-sm-flex align-items-center">
                <div>
                  <h6 class="font-weight-semibold text-lg mb-0">Sliders</h6>
                ...
                <div class="ms-auto d-flex">
                  ...
                  <a class="btn-inner--text text-white" href="{{ route('sliders.create') }}">
                ...
                <table class="table align-items-center mb-0">
                  <thead class="bg-gray-100">
                    <tr>
                      ...
                      <th class="text-secondary text-xs font-weight-semibold opacity-7 ps-2">Sort number</th>
...
<tr>
  <td>
    <div class="d-flex px-2 py-1">
        ...
        <h6 class="text-md text-dark font-weight-semibold mt-2 me-2">{{ $slider->firstItem() + $key }}</h6>
   

      <div class="d-flex align-items-center">
        <img src="{{ $row->image ? asset($row->image) : asset('/storage/setting/no_image.jpg')  }}" class="avatar avatar-md rounded-circle me-2" alt="{{ $row->title }}">
      </div>
      <div class="d-flex flex-column justify-content-center ms-1">
        <h6 class="mb-0 text-sm font-weight-semibold">{{ $row->title }}</h6>
        <p class="text-sm text-secondary mb-0">/{{ $row->id }}/{{ $row->slug }}</p>
      </div>
      {{-- <div class="my-auto">
  ...
  <td>
    <p class="mb-0 text-sm font-weight-normal">{{ $row->sort_number }}</p>
    </td>
  ...
  <td class="align-middle">
    {{-- <a href="{{ route('sliders.show', $row->id) }}" class="text-secondary font-weight-bold text-xs" data-bs-toggle="tooltip" data-bs-title="Show page">
      <i class="fa-regular fa-eye  text-primary" style="font-size: 0.9rem !important;" ></i>
    </a> --}}
    <a href="{{ route('sliders.edit', $row->id) }}" class="text-secondary font-weight-bold text-xs" data-bs-toggle="tooltip" data-bs-title="Edit page">
      <i class="fa-regular fa-pen-to-square text-primary mx-4" style="font-size: 0.9rem !important;"></i>
    </a>

    <form method="POST" action="{{ route('sliders.destroy', $row->id) }}" style="display: inline-block;">
      @csrf
      @method('DELETE')
    ...
    {{-- <a href="{{ route('sliders.delete', $row->id) }}" class="text-secondary font-weight-bold text-xs" data-bs-toggle="tooltip" data-bs-title="Edit user">
